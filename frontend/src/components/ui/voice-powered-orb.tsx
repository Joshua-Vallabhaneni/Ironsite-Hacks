...
export const VoicePoweredOrb: FC<VoicePoweredOrbProps> = ({
  ...
  const stopMicrophone = () => {
    ...
    } catch (error) { console.warn('Error stopping microphone:', error); }
  };
  ...
  const initMicrophone = async () => {
    try {
      ...
      return true;
    } catch (error) { console.warn("Microphone access denied:", error); return false; }
  };
    ...
    try {
      ...
      return () => {
        ...
        if (container && glContext && glContext.canvas) {
          try { if (container.contains(glContext.canvas)) container.removeChild(glContext.canvas); } catch (e) { console.warn(e); }
        }
        stopMicrophone();
  ...
  useEffect(() => {
    ...
    const handleMicrophoneState = async () => {
      if (enableVoiceControl) { await initMicrophone(); }
      else { stopMicrophone(); }
    };
